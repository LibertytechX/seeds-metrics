...
func (r *DashboardRepository) GetEarlyIndicatorLoans(filters map[string]interface{}) ([]*models.EarlyIndicatorLoan, error) {
	query := `
		SELECT
			...
			COALESCE(l.total_outstanding, 0) as amount_due,
			COALESCE(l.total_principal_paid, 0) + COALESCE(l.total_interest_paid, 0) + COALESCE(l.total_fees_paid, 0) as amount_paid,
			COALESCE(l.principal_outstanding, 0) as outstanding_balance,
			l.channel,
			l.status,
			l.fimr_tagged as fimr_tagged,
...
func (r *DashboardRepository) GetAllLoans(filters map[string]interface{}) ([]*models.AllLoan, int, error) {
	...
	query := `
		SELECT
			...
			COALESCE(l.interest_outstanding, 0) as interest_outstanding,
			COALESCE(l.fees_outstanding, 0) as fees_outstanding,
			COALESCE(l.total_outstanding, 0) as total_outstanding,
			COALESCE(l.actual_outstanding, 0) as actual_outstanding,
			COALESCE(l.total_repayments, 0) as total_repayments,
			l.status,
			l.django_status,
...
func (r *DashboardRepository) GetTopRiskLoans(officerID string, limit int) ([]*models.TopRiskLoan, error) {
	query := `
		SELECT
			...
			l.current_dpd,
			l.max_dpd_ever,
			COALESCE(l.total_outstanding, 0)::float as total_outstanding,
			COALESCE(l.principal_outstanding, 0)::float as principal_outstanding,
			COALESCE(l.interest_outstanding, 0)::float as interest_outstanding,
			COALESCE(l.fees_outstanding, 0)::float as fees_outstanding,
			l.status,
			l.fimr_tagged,
			l.channel,
	...
	query := `
		SELECT
				...
				COUNT(*) AS loans,
					...
					COALESCE(SUM(CASE WHEN l.django_status IN ('OPEN', 'PAST_MATURITY') THEN l.daily_repayment_amount ELSE 0 END), 0) AS daily_target,
