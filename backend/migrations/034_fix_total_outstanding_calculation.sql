-- ============================================================================
-- Migration 034: Fix total_outstanding Calculation
-- ============================================================================
-- Description: Fixes the total_outstanding calculation to use the simple
--              formula: repayment_amount - total_repayments
--              This prevents negative outstanding balances when repayments
--              exceed the principal amount.
--
-- Root Cause: The previous calculation (principal_outstanding + 
--             interest_outstanding + fees_outstanding) breaks down when
--             all repayments are allocated to principal, causing
--             principal_outstanding to go negative.
--
-- Solution: Use the straightforward formula:
--           total_outstanding = repayment_amount - total_repayments
--
...
BEGIN
    ...
    -- ========================================================================
    -- STEP 1: Recalculate all computed fields
    -- ========================================================================
    ...
    calculated_fields AS (
        SELECT
            ...
            -- ================================================================
            -- FIXED: Outstanding balances calculation
            -- ================================================================
            -- Keep individual component tracking for reference, but use
            -- simple formula for total_outstanding
            ...
            -- CORRECTED FORMULA: total_outstanding = repayment_amount - total_repayments
            -- This prevents negative values when repayments exceed principal
            GREATEST(0, COALESCE(lrd.repayment_amount, 0) - lrd.total_repayments) as total_outstanding,
            ...
            GREATEST(0,
                CASE
                    WHEN lrd.loan_term_days > 0 AND lrd.disbursement_date IS NOT NULL THEN
                        (
                            -- Total expected repayment (use repayment_amount if available, otherwise calculate)
                            COALESCE(lrd.repayment_amount,
                                lrd.loan_amount +
            ...
            CASE
                WHEN lrd.loan_term_days > 0 AND COALESCE(lrd.repayment_amount, 0) > 0 THEN
            ...
            GREATEST(0,
                CASE
                    WHEN lrd.first_payment_due_date IS NOT NULL AND lrd.loan_term_days > 0 AND COALESCE(lrd.repayment_amount, 0) > 0 THEN
                        CASE
                            WHEN LEAST(CURRENT_DATE, COALESCE(lrd.maturity_date, CURRENT_DATE)) >= lrd.first_payment_due_date THEN
                                count_business_days(
                                    lrd.first_payment_due_date,
...
'Recalculates all computed fields for all loans. Uses corrected total_outstanding formula: repayment_amount - total_repayments. Returns total loans processed, loans updated, and execution time.';
