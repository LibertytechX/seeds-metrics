-- Migration: Add total_repayments column to loans table
-- This migration adds a new computed field that stores the sum of all non-reversed repayments

-- Step 1: Add the column to the loans table
ALTER TABLE loans ADD COLUMN IF NOT EXISTS total_repayments DECIMAL(15, 2) DEFAULT 0;

-- Step 2: Create index for the new column
CREATE INDEX IF NOT EXISTS idx_loans_total_repayments ON loans(total_repayments);

-- Step 3: Update the trigger function to calculate total_repayments
...
-- Step 4: Populate the total_repayments field for existing loans
UPDATE loans l
SET total_repayments = COALESCE(
