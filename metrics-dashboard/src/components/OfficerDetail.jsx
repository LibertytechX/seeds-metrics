...
const OfficerDetail = ({
  ...
  const officerLabel = officerName || officerId || 'Officer';
  ...
  // Fetch dropdown options (regions, products/loan types, waves)
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        ...
        console.error('Error fetching Officer Detail filter options:', err);
      } finally {
  ...
  // Fetch branch info to get canonical branch name/region and MTD rate
  useEffect(() => {
    const fetchBranchInfo = async () => {
      ...
      try {
        ...
        const djangoStatusFilter =
          filters.period === 'today_only'
            ? 'OPEN'
            : ['OPEN', 'PAST_MATURITY', MISSING_VALUE].join(',');
        params.set('django_status', djangoStatusFilter);

        const res = await fetch(
          `${API_BASE_URL}/collections/branches?${params.toString()}`,
        );
        ...
        if (data.status === 'success' && data.data?.branches) {
          const match = data.data.branches.find(
            (b) =>
              b.branch &&
              b.branch.toLowerCase().replace(/\s+/g, '-') === branchSlug,
          );
          if (match) {
            setBranchInfo(match);
        ...
        console.error('Error fetching officer branch info:', err);
      }
    };

    fetchBranchInfo();
  }, [API_BASE_URL, branchSlug, branchName, filters.region, filters.product, filters.wave, filters.period]);

  // Fetch officer-scoped summary metrics
  useEffect(() => {
    const fetchSummaryMetrics = async () => {
      if (!branchFilterValue || !officerId) return;
      try {
        ...
        baseParams.set('officer_id', officerId);
        ...
        if (unrestrictedData.status === 'success') {
          setTotalRepaidAll(
            unrestrictedData.data?.summary_metrics?.total_repayments_today ??
              null,
          );
        }
      } catch (err) {
        console.error('Error fetching officer summary metrics:', err);
        setError(err.message || 'Error fetching officer data');
      } finally {
    ...
    fetchSummaryMetrics();
  }, [API_BASE_URL, branchFilterValue, officerId, filters.region, filters.product, filters.period, filters.wave]);

  // Fetch officer daily collections series
  useEffect(() => {
    const fetchDailyCollections = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/collections/daily?${params.toString()}`,
        );
        const data = await res.json();
        ...
        console.error('Error fetching officer daily collections:', err);
      } finally {
  ...
  }, [API_BASE_URL, branchFilterValue, officerId, filters.region, filters.product, filters.wave]);

  // Fetch officer loans (simple paged list, max 100 loans)
  useEffect(() => {
    const fetchLoans = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/loans?${params.toString()}`);
        const data = await res.json();
        if (data.status === 'success') {
        ...
        console.error('Error fetching officer loans:', err);
        setLoans([]);
      } finally {
    ...
    fetchLoans();
  }, [API_BASE_URL, branchFilterValue, officerId, filters.region, filters.product, filters.wave, filters.period]);
  ...
  const pastMaturityOutstanding =
    summaryMetrics?.past_maturity_outstanding ?? 0;
  ...
  // Risk snapshot percentages (approximate NPL from critical_loans / total_loans)
  ...
  const performingPercent =
    totalLoans > 0 ? (performingCount / totalLoans) * 100 : 0;
  const atRiskPercent = totalLoans > 0 ? (atRiskCount / totalLoans) * 100 : 0;
  const nplPercent = totalLoans > 0 ? (criticalCount / totalLoans) * 100 : 0;

  const mtdRate = branchInfo?.mtd_rate ? branchInfo.mtd_rate * 100 : 0;

  // Daily collections chart series (last 7 days)
    ...
    <div className="branch-detail-page">
      ...
      <div className="branch-detail-header">
        <div className="branch-detail-breadcrumb">
          ...
          >
            {branchInfo?.branch || branchName || branchSlug}
          </button>
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-current">{officerLabel}</span>
        </div>
      ...
      <div className="branch-detail-cards">
        ...
        <div className="branch-kpi-card">
          ...
          <div className="kpi-value">
            {formatShortCurrency(totalDueToday)} ·
            {' '}
            {formatShortCurrency(totalRepaidToday)}
          </div>
        </div>
        <div className="branch-kpi-card">
      ...
      <div className="branch-detail-chart-section">
        <h3>Officer Daily Collections vs Due</h3>
              ...
              >
                ...
                <RechartsTooltip
                  formatter={(value, name) => [
                    ...
                    name === 'collected_amount' ? 'Collected' : 'Due',
                  ]}
      ...
      <div className="branch-detail-risk-section">
        ...
        <div className="risk-snapshot-grid">
          ...
          <div className="risk-item">
            <span className="risk-label">NPL (approx):</span>
